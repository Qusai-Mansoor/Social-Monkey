...
class DashboardApp {
    constructor() {
        ...
        this.autoRefreshManager = window.autoRefreshManager || new AutoRefreshManager();
        this.filterPanel = window.filterPanel || new FilterPanel(this.filterManager);
    ...
    setupAutoRefresh() {
        ...
        this.autoRefreshManager.onRefresh(async (event, data) => {
            if (event === 'refresh' && this.currentDashboard && this.currentDashboard.render) {
        ...
        return `
            <div class="phase4-controls" style="display: flex; align-items: center; gap: 12px;">
                ...
                <button id="toggleAutoRefresh" class="btn-secondary" title="Toggle Auto-Refresh">
                    ...
                    ${autoRefreshStatus.enabled ? 'Disable' : 'Enable'} Auto-Refresh
                </button>
    ...
    async navigateToPage(pageName) {
        ...
        if (this.currentPage === pageName && this.currentDashboard) {
            return;
        }
    ...
    async loadConnectedAccounts() {
        try {
            ...
            if (accounts.length === 0) {
                container.innerHTML = '<p class="no-data">No accounts connected yet</p>';
                return;
            }
