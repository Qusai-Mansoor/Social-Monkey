...
class ApiService {
  ...
  async getTopPosts(limit = 5) {
    try {
      ...
      const posts = await this.request(
        `/api/v1/ingestion/posts?limit=${limit}`
      );
  ...
  async ingestData(accountId, maxPosts = 100) {
    return this.request(
      API_ENDPOINTS.INGEST(accountId) + `?max_posts=${maxPosts}`,
      {
        method: "POST",
      }
    );
  ...
  async getTopPosts(limit = 5) {
    try {
      ...
      const response = await this.request(
        `/api/v1/analytics/top-posts?limit=${limit}`
      );
  ...
  /**
   * Get negative triggers based on comment analysis
   * @param {number} dateRange - Number of days to look back
   * @param {string} platform - Platform filter ('all', 'twitter', 'facebook')
   * @param {string} severity - Severity filter ('all', 'high', 'medium', 'low')
   */
  async getNegativeTriggers(
    dateRange = 30,
    platform = "all",
    severity = "all"
  ) {
    try {
      ...
      return await this.request(
        `/api/v1/analytics/dashboard/negative-triggers?${params}`
      );
    } catch (error) {
      console.error("Error fetching negative triggers:", error);
      return {
        negativeTriggers: [],
        topPosts: [],
        stats: {
  ...
  /**
   * Get negative comments for a specific post
   * @param {number} postId - The ID of the post
   */
  async getPostNegativeComments(postId) {
    try {
      return await this.request(
        `/api/v1/analytics/post-negative-comments/${postId}`
      );
    } catch (error) {
  ...
  async getPostComments(postId) {
    try {
      return await this.request(`/api/v1/analytics/post-comments/${postId}`);
    } catch (error) {
